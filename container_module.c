...
{
	char *cont_type_str = calloc(11, 1);
...
static void forward_concat_mod(module *cm)
{
	...
	tensor out_vol;
	tensor *mod_outs = calloc(cm->n_modules, sizeof(tensor));
	...
	out_vol.data = calloc(out_vol.d*out_vol.h*out_vol.w, sizeof(tensor_data_t));
...
module init_container_mod(container_t cont_type, int n_modules, int concat_dim)
{
	...
	new_container.modules = calloc(new_container.n_modules, sizeof(module));
	
	// TEST
	_mem_alloc_by_model += new_container.n_modules*sizeof(module);
