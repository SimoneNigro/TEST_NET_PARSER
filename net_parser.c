...
/*static*/ tensor_data_t *parse_tensor_data(const char *str, int n_data)
{
	...
	data = calloc(n_data, sizeof(tensor_data_t));
...
static module parse_convolutional_mod(mxml_node_t *conv_node)
{
	...
	return init_module(CONVOLUTIONAL, N_INIT_ARG_CONV, n_fil, ker_h, ker_w, 
...
static module parse_module(mxml_node_t *curr_node)
{
	...
	else if(strcmp(type, "relu") == 0 || strcmp(type, "tanh") == 0 || strcmp(type, "sigmoid") == 0) /* || strcmp(type, "other_activation_type") == 0*/
	{
		...
		parsed = init_module(ACTIVATION, N_INIT_ARG_ACTIV, act_type);		
	}
	else if(strcmp(type, "spatialmaxpooling") == 0)
	{
		...
		parsed = init_module(POOL, N_INIT_ARG_POOL, ker_h, ker_w, pad_h, pad_w, stride_h, stride_w);
	}
	else parsed.type = 0;
...
static module parse_container(mxml_node_t *container_node)
{
	...
	container = init_module(CONTAINER, N_INIT_ARG_CONT, cont_type, n_modules, dim);
...
{
	FILE *to_parse = fopen(to_parse_fn, "r");

	if(!to_parse)
	{
		fprintf(stderr, "ERROR: Can't open \"%s\" file. Get sure the file's path and name are correct. Exiting...\n", to_parse_fn);
		exit(-1);
	}
