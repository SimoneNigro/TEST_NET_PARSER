...
tensor init_tensor(int d, int w, int h, int rand_init)
{
	...
	new_tensor.data = calloc(d * h * w, sizeof(tensor_data_t));
	
	// TEST
	//_mem_alloc_by_model += d*w*h*sizeof(tensor_data_t);
...
/*static*/ tensor pad_tensor(tensor src, int pad_h, int pad_w)
{
	...
	// TEST
	//_mem_alloc_by_model += (padded.d*padded.w*padded.h - src.d*src.w*src.h)*sizeof(tensor_data_t);
